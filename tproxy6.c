...
int main(int argc, char** argv)
{
  ...
  inet_pton(AF_INET6, argv[1], &bind_addr.sin6_addr);
  bind_addr.sin6_port = htons(TPROXY_BIND_PORT);
  ...
  while (1)
  {
    ...
    // talking to.
    struct sockaddr_in6 client_addr, intended_dest_addr;
    ...
    printf("accepted socket from %s:%d; ",
           inet_ntop(AF_INET6, &client_addr.sin6_addr,str6,sizeof(str6)),
           ntohs(client_addr.sin6_port));
    printf("they think they're talking to %s:%d\n",
           inet_ntop(AF_INET6, &intended_dest_addr.sin6_addr,str6,sizeof(str6)),
           ntohs(intended_dest_addr.sin6_port));
